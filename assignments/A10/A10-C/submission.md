...
- title: A10-C submission

## question 1

Used sounds:

- `A10-b-1.wav` is re-synthesized from: Small String Orchestra Tuning by unfa -- <https://freesound.org/s/170857/> -- License: Creative Commons 0
- `A10-b-2.wav` is re-synthesized from: orchestra tuning+applause by FrancoD -- <https://freesound.org/s/739048/> -- License: Creative Commons 0

Both sounds are similar in the instruments featured \(except for the clapping sound for the second sound\), so I use the same parameters for both sounds

- \[0 Hz, 500 Hz\): M=5292, N=8192
- \[500 Hz, 2500 Hz\): M=2646, N=4096
- \[2500 Hz, 22050 Hz\): M=1323, N=2048

A Blackman window is used for its relatively low noise and acceptable main lobe size. The middle window size \(M=2646\) is chosen by noticing that 100 Hz is a good resolution for the middle band. So we calculate 6\*44100/100=2646. The remaining two bands are derived by respectively multiplying and dividing this number by 2. The band boundaries are chosen by inspecting the spectrogram, and find that these boundaries can somewhat segment the spectrogram \(though the boundaries are not very clear\).

Comparing with the sounds re-synthesized using the normal sine model, I see the spectrogram using multi-resolution can capture more sinusoids at higher frequencies. This is because shorter notes at high frequency can be captured by a smaller window size at higher frequencies. The clapping sound, which is hard to re-synthesize well, is reproduced better using multi-resolution. This shows the time—frequency tradeoff is balanced better in the multi-resolution model. The drawback is that the computation complexity is much higher. This is because we need to apply DFT 3 times per frame with the multi-resolution model.

If I were to extend it to the HPR and/or HPS model, the challenge would be the method to identify fundamental frequencies and harmonics across the 3 bands. Also, the harmonics can change in between bands over time, so we also need to take this into account.

---

- file: [submission.pdf](submision.pdf)
...
- file: [part 2.zip](part%202.zip)
  - source: [part 2/](part%202/)
    - [170857__unfa__small-string-orchestra-tuning.flac](part%202/170857__unfa__small-string-orchestra-tuning.flac)
    - [739048__francod__orchestra-tuningapplause.wav](part%202/739048__francod__orchestra-tuningapplause.wav)
    - [A10-b-1-sineModel.wav](part%202/A10-b-1-sineModel.wav)
    - [A10-b-1-source.wav](part%202/A10-b-1-source.wav)
    - [A10-b-1.wav](part%202/A10-b-1.wav)
    - [A10-b-2-sineModel.wav](part%202/A10-b-2-sineModel.wav)
    - [A10-b-2-source.wav](part%202/A10-b-2-source.wav)
    - [A10-b-2.wav](part%202/A10-b-2.wav)
...
- file: [sineModelMultiRes.py](sineModelMultiRes.py)
