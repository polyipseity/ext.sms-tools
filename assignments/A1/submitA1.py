...
def loginPrompt():
    ...
    password = input(
        "Submission Token (from the assignment page. This is NOT your own account's password): "
    )
...
def partPrompt():
    ...
    partIdx = input(
        "Please enter which part you want to submit (1-" + str(counter) + " or all): "
    )
    try:
...
def get_parts(partIdx, output):
    ...
    output = (
        output + "\n\n\n" + source(partIdx)
    )  # concatenating with the source code for log
...
def submitSolution_all_parts(email_adress, secret):
    ...
    for idx in range(len(LIST_PARTIDS)):
        ...
        if out == "":
            ...
            email.encoders.encode_base64(output_64_msg)
            outputs.append(
                output_64_msg.get_payload() + "\n\n\n" + source(idx)
            )  # concatenating the source code for log  # type: ignore
...
def send_request(values):
    req = (
        'curl -s -X POST -H "Cache-Control: no-cache" -H "Content-Type: application/json" -d '
        + "'"
...
def send_request_new(values):
    ...
    try:
        ...
        try:
            ...
            raise Exception(
                'Something got wrong. Try installing Requests module and retry ("pip install requests")'
            )
...
def convertNpObjToStr(obj):
    ...
    if isinstance(obj, np.ndarray):
        obj = np.ascontiguousarray(
            obj
        )  # this is a very important step, because many times the np object doesn't have its content in continous memory
...
def wrongOutputTypeError(outType, part):
    print(
        "\n Type error in Part "
        + str(part + 1)
        + " - The output data type of your function doesn't match the expected data type: ("
        + str(outType)
        + ")."
    )
    # print "== Submission failed: Please correct and resubmit."
