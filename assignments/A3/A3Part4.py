...
def suppressFreqDFTmodel(x: NDArray[np.float64], fs: float, N: int):
    """
    Inputs:
    ...
    mX, pX = dftAnal(x, w, N)
    mX2 = mX.copy()
    mX2[: ceil(70 / fs * N) + 1] = -120

    return (
        dftSynth(mX, pX, M) * outputScaleFactor,
        dftSynth(mX2, pX, M) * outputScaleFactor,
    )
